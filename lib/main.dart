...
class _DeliveryScreenState extends State<DeliveryScreen> {
          ...
          ElevatedButton(
            onPressed: () {
              if (tempController.text.isNotEmpty) {
                ...
                final foundItems = _items.where(
                  (item) => item.barcode == tempController.text,
                );
                if (foundItems.isNotEmpty) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text(
                        'بارکد ثبت شد، لطفاً اطلاعات کالا را کامل کنید',
                      ),
  ...
  void _showAddDialog() {
    ...
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('اضافه کردن کالا', textAlign: TextAlign.center),
        content: Form(
          key: _formKey,
                    ...
                    Expanded(
                      child: DropdownButtonFormField<String>(
                        ...
                        items: const [
                          ...
                          DropdownMenuItem(value: 'ریال', child: Text('ریال')),
                        ],
                        onChanged: (value) {
                          setState(() {
        ...
        actions: [
          ...
          ElevatedButton(onPressed: _addItem, child: const Text('افزودن')),
        ],
      ),
    );
  }
  ...
  Future<void> _loadSavedManifests() async {
    ...
    if (manifestsJson != null) {
      try {
        ...
        setState(() {
          _savedManifests = decoded
              .map((item) => DeliveryManifest.fromJson(item))
              .toList();
          _isLoading = false;
        });
      } catch (e) {
        ...
        actions: [
          ...
          TextButton(
            ...
            onPressed: () async {
              ...
              final prefs = await SharedPreferences.getInstance();
              final manifestsJson = _savedManifests
                  .map((m) => m.toJson())
                  .toList();
              await prefs.setString(
                'delivery_manifests',
                jsonEncode(manifestsJson),
              );
      ...
      children: [
        ...
        if (_isSearching)
          Padding(
            ...
            child: Text(
              '${_filteredItems.length} نتیجه از ${_items.length} کالا',
              style: const TextStyle(color: Colors.grey, fontSize: 14),
            ),
          ),
        Expanded(
          child: _items.isEmpty && _savedManifests.isEmpty
              ? const Center(
                  child: Column(
              ...
              : ListView.builder(
                  itemCount: _items.isNotEmpty
                      ? (_isSearching ? _filteredItems.length : _items.length)
